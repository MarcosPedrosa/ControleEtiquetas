...
If Grid1.Rows > 0 Then
   ...
   If Format(Trim(Grid1.Text), "DD/MM/YYYY") = "01/01/1900" Then
'      rs.Fields("03_Data_Producao").Value = " "
      Grid1.col = 21 '22_Num_Lote(este campo é preenchido com a data de producao)
      sDateAux = Trim(Grid1.Text)
      If IsDate(sDateAux) Then
         rs.Fields("03_Data_Producao").Value = Format(Trim(Grid1.Text), "DD/MM/YYYY")
      Else
         rs.Fields("03_Data_Producao").Value = Format(Now(), "DD/MM/YYYY")
      End If
   Else
      sDateAux = Trim(Grid1.Text)
      If IsDate(sDateAux) Then
         rs.Fields("03_Data_Producao").Value = Trim(Grid1.Text)
      Else
         rs.Fields("03_Data_Producao").Value = Format(Now(), "DD/MM/YYYY")
      End If
   End If
...
'********************************************************************************************************
   ...
   Grid1.col = 33: rs.Fields("34_DUM").Value = Trim(Grid1.Text)
   If Format(Trim(Grid1.Text), "DD/MM/YYYY") = "01/01/1900" Then
      rs.Fields("34_DUM").Value = " "
   Else
      rs.Fields("34_DUM").Value = Trim(Grid1.Text)
   End If
...
Rem Codigos especiais(24)- Código de Barras
       ...
       .lbl_37_sequencial.Caption = Mid$(Trim(rs.Fields("ID_ETIQUETA").Value), Len(Trim(rs.Fields("ID_ETIQUETA").Value)) - 3, Len(Trim(rs.Fields("ID_ETIQUETA").Value)))
...
'*************************************************************************
        ...
        ts.WriteLine .sTextoQrCode
        
       .lbl_24_Codigo_Barra_A.Caption = "04" & sNumDesenho & _
                                        Format(rs.Fields("11_Cod_Fornecedor").Value, "0000000") & _
                                        Format(rs.Fields("07_Quantidade").Value, "000000") & _
                                        Mid(Trim(rs.Fields("06_Cod_Emb").Value), 1, 4)
        
        
        ts.WriteLine .lbl_24_Codigo_Barra_A
        ts.Close
        Set ts = Nothing
        ...
        Do While bExiste = False
           If Dir$(Imagem) <> "" Then bExiste = True
        Loop
        .IMG_CODE_128.Picture = LoadPicture(Imagem)
...
For nx = 1 To cRec.RecordCount
    ...
    If Val(cRec.Fields("ID_CLIENTE")) = 211 Then ' "0000000211" Then
       Grid1.col = 27: Grid1.Text = "FPTBE" 'IIf(IsNull(cRec.Fields("Destino")), " ", cRec.Fields("Destino"))
       Grid1.col = 28: Grid1.Text = "196" 'IIf(IsNull(cRec.Fields("Cod_Destino")), " ", cRec.Fields("Cod_Destino"))
    Else
       Grid1.col = 27: Grid1.Text = "CKD" 'IIf(IsNull(cRec.Fields("Destino")), " ", cRec.Fields("Destino"))
       Grid1.col = 28: Grid1.Text = "000" 'IIf(IsNull(cRec.Fields("Cod_Destino")), " ", cRec.Fields("Cod_Destino"))
    End If
'   Grid1.col = 27: rs.Fields("28_Destino").Value = Mid$(Trim(Grid1.Text), 1, 20)
